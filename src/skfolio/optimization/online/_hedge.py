...
class ExponentialGradient(BaseOptimization):
    """Hedge (Multiplicative Weights) estimator.

    The Hedge algorithm computes next-period portfolio weights by
    multiplying current weights by each asset's gross return and
    renormalizing. With uniform initialization, this yields weights
    proportional to each asset's cumulative gross return over the
    observation window.

    If ``X`` contains arithmetic returns, we internally use gross returns
    ``1 + r_t`` and require them to be strictly positive.
    ...
    def fit(
        ...
        """Fit the Hedge (Multiplicative Weights) estimator.
        ...
        # Cumulative wealth per asset under buy-and-hold of each single asset
        # Hedge with uniform initialization yields weights proportional to these
