...
def test_methods_basic_validity_fit(method, X_small):
    ...
    est = FTRLProximal(objective=method)
    est.fit(X_small)
    _check_box_budget(est.weights_, 0.0, 1.0, 1.0)
...
def test_smooth_prediction(X_small):
    ...
    est_vanilla = FTRLProximal(objective=FTRLStrategy.EG, learning_rate=0.1)
    est_smooth = FTRLProximal(
        objective=FTRLStrategy.EG, learning_rate=0.1, smooth_prediction=True
    )
...
def test_warm_start_and_initial_weights(X_small_single):
    ...
    est_no_warm = FTRLProximal(
        objective=FTRLStrategy.EG, initial_weights=init, warm_start=False
    )
    ...
    est_warm = FTRLProximal(
        objective=FTRLStrategy.EG, initial_weights=init, warm_start=True
    )
...
def test_partial_fit_streaming_equivalence(X_small):
    est_batch = FTRLProximal(objective=FTRLStrategy.EG, learning_rate=0.3)
    est_batch.fit(X_small)

    est_stream = FTRLProximal(objective=FTRLStrategy.EG, learning_rate=0.3)
...
def test_eg_tilde_implementation(X_small):
    ...
    est_uniform = FTRLProximal(
        objective=FTRLStrategy.EG, eg_tilde=True, eg_tilde_alpha=1.0
    ).fit(X_small)
    ...
    est_eg = FTRLProximal(objective=FTRLStrategy.EG, eg_tilde=False).fit(X_small)
    est_no_mix = FTRLProximal(
        objective=FTRLStrategy.EG, eg_tilde=True, eg_tilde_alpha=0.0
    ).fit(X_small)
    ...
    est_callable = FTRLProximal(
        objective=FTRLStrategy.EG, eg_tilde=True, eg_tilde_alpha=alpha_schedule
    ).fit(X_small)
...
def test_ftrl_vs_omd_mode(method, X_small):
    ...
    # OMD mode (default)
    est_omd = FTRLProximal(objective=method, ftrl=False)
    est_omd.fit(X_small)
    _check_box_budget(est_omd.weights_, 0.0, 1.0, 1.0)

    # FTRL mode
    est_ftrl = FTRLProximal(objective=method, ftrl=True)
...
def test_learning_rate_callable(X_small):
    ...
    est_const = FTRLProximal(objective=FTRLStrategy.EG, learning_rate=0.1).fit(X_small)
    _check_box_budget(est_const.weights_, 0.0, 1.0, 1.0)

    # Equivalent callable
    est_callable_equiv = FTRLProximal(
        objective=FTRLStrategy.EG, learning_rate=lambda t: 0.1
    ).fit(X_small)
    ...
    est_callable_varied = FTRLProximal(
        objective=FTRLStrategy.EG, learning_rate=lr_fn
    ).fit(X_small)
...
def test_static_regret_against_best_crp():
    ...
    est = FTRLProximal(
        objective=FTRLStrategy.EG, ftrl=True, learning_rate=0.2, warm_start=False
    )
...
def test_partial_fit_input_and_warnings_on_sample_weight_and_nonpositive_return():
    ...
    est = FTRLProximal(
        objective=FTRLStrategy.EG, ftrl=False, learning_rate=0.2, warm_start=False
    )
...
def test_objective_not_implemented_raises_value_error():
    ...
    X = make_stationary_returns(T=5, gap=0.01, n=2)
    est = FTRLProximal(
        objective=FTRLStrategy.FOLLOW_THE_LEADER,
        ftrl=False,
        learning_rate=0.2,
        warm_start=False,
    )
...
def test_warm_start_reset_vs_non_warm_behavior():
    ...
    est_warm = FTRLProximal(
        objective=FTRLStrategy.EG, ftrl=False, learning_rate=0.2, warm_start=True
    )
    est_warm.fit(X)
